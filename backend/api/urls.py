...
urlpatterns = [
    ...
    path('user/<int:user_id>', user_detail, name='profile'),
    path('event/', get_events, name='event-list-create'),
    path('event/<int:pk>/', event_detail, name='event-detail'),
    path('event/<int:event_id>/join/', join_event, name='join-event'),
    path('event/<int:event_id>/leave/', leave_event, name='leave-event'),
